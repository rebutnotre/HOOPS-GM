...
function generateBoxScores(players: Player[], teamPoints: number, rng: () => number, lineup?: string[]): Map<string, Partial<PlayerStats>> {
  ...
  active.forEach((p, i) => {
    ...
    // 3PT%: tied directly to shooting3 rating. rating 30→24%, 65→32%, 99→40%
    const tend3Pct  = clamp(0.24 + (p.ratings.shooting3 - 30) / 69 * 0.16, 0.19, 0.40);
    const game3Pct  = clamp(randNormal(tend3Pct, 0.045, rng), 0.12, 0.44);
    // 2PT%: weighted blend of finishing (paint), midRange, and scoring per user request. rating 30→34%, 65→45%, 99→58%
    const raw2Rating = p.ratings.finishing * 0.40 + p.ratings.midRange * 0.35 + p.ratings.scoring * 0.25;
    const base2Pct  = clamp(0.34 + (raw2Rating - 30) / 69 * 0.24, 0.31, 0.58);
    const game2Pct  = clamp(randNormal(base2Pct, 0.05, rng), 0.22, 0.65);
    ...
    // Always ensure at least 1 miss when player has taken 2+ shots
    finalFga  = clamp(Math.max(finalFga, finalFgm + (finalFgm >= 2 ? 1 : 0)), 0, 40);
